function CI_v01(MonteCarlo,L,K,M,PdBTxRx,Init,Iter,GTS,sigma2n,Name1,Name2,InputFolder)
tgGlobal=tic;
%AlgNameA='Linear_Joint_Init_Iter_DL_BCC';                        % Name of the joint design algorithm
...
[CodeFolder,~,~,AlgNameA]=CodeFolder_SheetName_DataFolder_AlgName_v01(mfilename);  % Locations of the code and data folders. Beam selection algorithm name retrieved from the file name
...
while mc<=MonteCarlo
    ...
    for initi=1:Init
        ...
        for stepi=1:Iter
            ...
            for l=0:L-1
                for k=0:K-1
                    ...
                    for m=1:kx
                        ...
                        rate_O(m)=sum(rate_UV_ZF_DLa_v01(sP,l,K,M,H,Usx,sigma2n)); % Evaluate rate based on ZF digital precoder
                    end
                    [~,index]=max(rate_O);
                    %%%
                    % BCC
            ...
            rate_Sum=sum(rate_UV_ZF_DLb_v01(sP,L,K,M,H,Usx,sigma2n)); % Evaluate final rate after all assignments are completed
